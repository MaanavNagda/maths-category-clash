/* Host role renderer (online) — control panel. Runs GameState locally and
   pushes snapshots to Firebase; receives full snapshots (answers, wagers,
   ranking) that the display never sees early. */

import { initOnline } from "./sync.js";
...
function viewSetup(s) {
  ...
  // display link — open this on the projector / second tab
  p.appendChild(el("h3", null, "Display"));
  const dRow = el("div", "row");
  const dUrl = controller.displayUrl();
  const dInp = document.createElement("input");
  dInp.type = "text";
  dInp.value = dUrl;
  dInp.readOnly = true;
  dInp.style.flex = "1";
  dInp.onclick = () => dInp.select();
  dRow.appendChild(dInp);
  const copyBtn = el("button", null, "Copy");
  copyBtn.onclick = () => {
    navigator.clipboard && navigator.clipboard.writeText(dUrl);
    copyBtn.textContent = "Copied";
    setTimeout(() => { copyBtn.textContent = "Copy"; }, 1200);
  };
  dRow.appendChild(copyBtn);
  const openBtn = el("button", null, "Open ↗");
  openBtn.onclick = () => window.open(dUrl, "_blank");
  dRow.appendChild(openBtn);
  p.appendChild(dRow);
  p.appendChild(el("p", "label",
    "Keep this tab private — it contains the answers. Share only the " +
    "display link (no key in it)."));
  ...
  // question files — three separate uploads
  ...
  img.src = s.logo || "placeholder-logo.svg";
...
initOnline("host", render, paintTimer);
